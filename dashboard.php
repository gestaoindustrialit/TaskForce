...
$statsStmt = $pdo->prepare(
    'SELECT
        (SELECT COUNT(*) FROM team_members WHERE user_id = ?) AS total_teams,
        (SELECT COUNT(*) FROM users) AS total_users,
        (SELECT COUNT(*) FROM projects p
         INNER JOIN team_members tm ON tm.team_id = p.team_id
         WHERE tm.user_id = ?) AS total_projects'
);
    ...
    <div class="row align-items-center g-3">
        ...
        <div class="col-lg-4">
            <div class="row g-2 text-center">
                <div class="col-4"><div class="stat-pill"><strong><?= (int) $stats['total_teams'] ?></strong><span>Equipas</span></div></div>
                <div class="col-4"><div class="stat-pill"><strong><?= (int) $stats['total_projects'] ?></strong><span>Projetos</span></div></div>
                <div class="col-4"><div class="stat-pill"><strong><?= (int) $stats['total_users'] ?></strong><span>Users</span></div></div>
            </div>
        </div>
    </div>
</section>
    ...
    <div class="col-lg-8">
        <div class="card shadow-sm soft-card h-100">
            <div class="card-header d-flex justify-content-between align-items-center bg-white border-0 pt-4 px-4">
                <h2 class="h4 mb-0">As tuas equipas</h2>
            ...
            <div class="card-body px-4 pb-4">
                <div class="row g-3">
                    <?php foreach ($teams as $team): ?>
                        <div class="col-md-6">
                            <div class="team-card h-100 p-3">
                                <h3 class="h5 mb-1"><?= h($team['name']) ?></h3>
                                <p class="text-muted small mb-2"><?= h($team['description']) ?: 'Sem descrição' ?></p>
                                <p class="small mb-3">Projetos: <strong><?= (int) $team['total_projects'] ?></strong> · Papel: <span class="badge text-bg-light border"><?= h($team['role']) ?></span></p>
    ...
    <div class="col-lg-4">
        <div class="card shadow-sm soft-card mb-4">
            <div class="card-header bg-white border-0 pt-4 px-4 d-flex justify-content-between align-items-center">
                ...
                <button class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#userModal">Novo user</button>
            </div>
            <div class="card-body px-4">
                <?php foreach ($users as $user): ?>
                    <div class="d-flex justify-content-between align-items-center border rounded p-2 mb-2">
                        <div>
                            <strong class="small"><?= h($user['name']) ?></strong>
                            <div class="small text-muted"><?= h($user['email']) ?></div>
                        </div>
                        <span class="small text-muted">#<?= (int) $user['id'] ?></span>
                    </div>
                <?php endforeach; ?>
            </div>
        </div>
        ...
        <div class="card shadow-sm soft-card">
            <div class="card-body p-4">
                <h3 class="h5">Atalho: pedidos internos</h3>
                <p class="small text-muted">Dentro do projeto podes gerar formulários para pedidos de tornearia, manutenção e compras.</p>
                <a href="dashboard.php" class="btn btn-outline-secondary btn-sm disabled">Disponível por projeto</a>
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="userModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <form class="modal-content" method="post">
            ...
            <div class="modal-footer">
                <button class="btn btn-primary">Criar utilizador</button>
            </div>
        </form>
    </div>
</div>
