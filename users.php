    ...
    if ($action === 'create_user') {
        ...
        if ($name === '' || $email === '' || $password === '') {
            ...
            try {
                ...
                $flashError = 'Não foi possível criar utilizador (email já em uso).';
            }
        }
    }
    ...
    if ($action === 'update_user') {
        ...
        if ($targetUserId <= 0 || $name === '' || $email === '') {
            ...
            try {
                ...
                $flashError = 'Não foi possível atualizar utilizador (email já em uso).';
            }
        }
    }
}
...
$usersStmt = $pdo->prepare('SELECT id, name, email, is_admin, created_at FROM users ORDER BY created_at DESC LIMIT ? OFFSET ?');
    ...
    <div class="card-body px-4 pb-4">
        ...
        <div class="table-responsive">
            <table class="table table-sm align-middle">
                <thead><tr><th>#</th><th>Nome</th><th>Email</th><th>Perfil</th><th>Criado</th><th></th></tr></thead>
                <tbody>
                <?php foreach ($users as $user): ?>
                    <tr>
                        ...
                        <td><?= h($user['name']) ?></td>
                        <td><?= h($user['email']) ?></td>
                        <td><?= (int) $user['is_admin'] === 1 ? '<span class="badge text-bg-dark">Admin</span>' : '<span class="badge text-bg-secondary">User</span>' ?></td>
