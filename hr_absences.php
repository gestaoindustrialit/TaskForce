...
$reasons = $pdo->query('SELECT code, label FROM shopfloor_absence_reasons WHERE is_active = 1 ORDER BY label COLLATE NOCASE ASC')->fetchAll(PDO::FETCH_ASSOC);
...
$pendingStmt = $pdo->query('SELECT a.id, a.start_date, a.end_date, a.reason, a.status, u.name AS user_name FROM shopfloor_absence_requests a INNER JOIN users u ON u.id = a.user_id WHERE a.status IN ("Pendente Nível 1", "Pendente RH") ORDER BY a.created_at ASC LIMIT 50');
$pending = $pendingStmt->fetchAll(PDO::FETCH_ASSOC);

$requestsStmt = $pdo->query('SELECT a.id, a.start_date, a.end_date, a.reason, a.details, a.status, a.created_at, u.name AS user_name FROM shopfloor_absence_requests a INNER JOIN users u ON u.id = a.user_id ORDER BY a.created_at DESC LIMIT 200');
...
<div class="card shadow-sm mb-3">
    <div class="card-body">
        ...
        <form method="post" class="row g-2">
            ...
            <div class="col-md-3"><label class="form-label">Colaborador</label><select class="form-select" name="user_id" required><?php foreach ($users as $u): ?><option value="<?= (int) $u['id'] ?>"><?= h((string) $u['name']) ?></option><?php endforeach; ?></select></div>
            ...
            <div class="col-md-3"><label class="form-label">Motivo</label><select class="form-select" name="reason_code" required><option value="">Selecione</option><?php foreach ($reasons as $r): ?><option value="<?= h((string) $r['code']) ?>"><?= h((string) $r['code'] . ' - ' . $r['label']) ?></option><?php endforeach; ?></select></div>
...
<div class="card shadow-sm">
    ...
    <div class="card-body">
        <div class="table-responsive">
            ...
            <?php foreach ($requests as $row): ?><tr><td><?= h((string) $row['user_name']) ?></td><td><?= h((string) $row['start_date']) ?> → <?= h((string) $row['end_date']) ?></td><td><span class="badge text-bg-light border"><?= h((string) $row['status']) ?></span></td><td><?= h((string) $row['reason']) ?></td><td><?= h((string) ($row['details'] ?? '')) ?></td></tr><?php endforeach; ?>
            </tbody></table>
        </div>
    </div>
</div>
