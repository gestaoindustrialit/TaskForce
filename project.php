...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create_subtask') {
        ...
        if ($parentId && $title !== '') {
            $stmt = $pdo->prepare(
                'INSERT INTO tasks(project_id, parent_task_id, title, status, created_by)
                 VALUES (?, ?, ?, "todo", ?)'
            );
    ...
    if ($action === 'send_daily_report') {
        ...
        $checkStmt = $pdo->prepare(
            'SELECT SUM(is_done) AS done_items, COUNT(*) AS total_items
        ...
        if (!$leaderEmail) {
            $leaderStmt = $pdo->prepare(
                'SELECT u.email
                 ...
                 WHERE tm.team_id = ? AND tm.role IN ("owner", "leader")
        ...
        if ($leaderEmail) {
            ...
            $body = "Projeto: {$project['name']}\n" .
                ...
                "Checklist: {$checkStats['done_items']}/{$checkStats['total_items']} itens concluídos\n";
...
<div class="d-flex justify-content-between align-items-start mb-3">
    ...
    <div class="d-flex gap-2">
        <form method="post">
            ...
            <button class="btn btn-outline-secondary">Enviar relatório diário</button>
        </form>
...
<?php if ($view === 'board'): ?>
    <div class="row g-3">
        <?php foreach (['todo' => 'Por Fazer', 'in_progress' => 'Em Progresso', 'done' => 'Concluídas'] as $status => $label): ?>
            <div class="col-md-4">
                <div class="card board-column shadow-sm">
