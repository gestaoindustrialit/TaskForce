...
<?php if ($view === 'board'): ?>
    <div class="row g-3">
        <?php foreach (['todo' => 'Por Fazer', 'in_progress' => 'Em Progresso', 'done' => 'Concluídas'] as $status => $label): ?>
            <div class="col-md-4"><div class="card board-column shadow-sm soft-card"><div class="card-header"><strong><?= h($label) ?></strong></div><div class="card-body vstack gap-2"><?php foreach ($tasks as $task): ?><?php if ($task['status'] !== $status) continue; ?><div class="task-card border rounded p-2 bg-white"><div class="d-flex justify-content-between align-items-start"><strong><?= h($task['title']) ?></strong><span class="badge bg-<?= task_badge_class($task['status']) ?>"><?= h(status_label($task['status'])) ?></span></div><p class="small text-muted mb-2"><?= h($task['description']) ?></p><form method="post" class="d-flex gap-2"><input type="hidden" name="action" value="change_status"><input type="hidden" name="task_id" value="<?= (int) $task['id'] ?>"><select name="status" class="form-select form-select-sm"><option value="todo" <?= $task['status'] === 'todo' ? 'selected' : '' ?>>Por Fazer</option><option value="in_progress" <?= $task['status'] === 'in_progress' ? 'selected' : '' ?>>Em Progresso</option><option value="done" <?= $task['status'] === 'done' ? 'selected' : '' ?>>Concluída</option></select><button class="btn btn-sm btn-outline-primary">Atualizar</button></form></div><?php endforeach; ?></div></div></div>
        <?php endforeach; ?>
    </div>
<?php else: ?>
    <div class="vstack gap-3">
        <?php foreach ($tasks as $task): ?>
            <div class="card shadow-sm soft-card"><div class="card-body"><div class="d-flex justify-content-between align-items-start mb-2"><div><h2 class="h5 mb-1"><?= h($task['title']) ?></h2><p class="text-muted mb-1"><?= h($task['description']) ?></p><small class="text-muted">Criada por <?= h($task['creator_name']) ?> · Prioridade <?= h($task['priority']) ?><?= $task['due_date'] ? ' · Prazo ' . h($task['due_date']) : '' ?></small></div><span class="badge bg-<?= task_badge_class($task['status']) ?>"><?= h(status_label($task['status'])) ?></span></div><div class="row g-3"><div class="col-md-4"><form method="post" class="d-flex gap-2 align-items-center"><input type="hidden" name="action" value="change_status"><input type="hidden" name="task_id" value="<?= (int) $task['id'] ?>"><select name="status" class="form-select form-select-sm"><option value="todo" <?= $task['status'] === 'todo' ? 'selected' : '' ?>>Por Fazer</option><option value="in_progress" <?= $task['status'] === 'in_progress' ? 'selected' : '' ?>>Em Progresso</option><option value="done" <?= $task['status'] === 'done' ? 'selected' : '' ?>>Concluída</option></select><button class="btn btn-sm btn-outline-primary">Guardar</button></form></div><div class="col-md-4"><form method="post" class="d-flex gap-2"><input type="hidden" name="action" value="create_subtask"><input type="hidden" name="parent_task_id" value="<?= (int) $task['id'] ?>"><input name="title" class="form-control form-control-sm" placeholder="Nova sub tarefa" required><button class="btn btn-sm btn-outline-secondary">+</button></form></div><div class="col-md-4"><form method="post" class="d-flex gap-2"><input type="hidden" name="action" value="add_checklist"><input type="hidden" name="task_id" value="<?= (int) $task['id'] ?>"><input name="content" class="form-control form-control-sm" placeholder="Item checklist" required><button class="btn btn-sm btn-outline-success">+</button></form></div></div><div class="row mt-3"><div class="col-md-6"><h3 class="h6">Sub Tarefas</h3><ul class="list-group list-group-flush"><?php foreach ($subtasksByParent[$task['id']] ?? [] as $subtask): ?><li class="list-group-item px-0 d-flex justify-content-between align-items-center"><?= h($subtask['title']) ?><span class="badge bg-<?= task_badge_class($subtask['status']) ?>"><?= h(status_label($subtask['status'])) ?></span></li><?php endforeach; ?><?php if (empty($subtasksByParent[$task['id']])): ?><li class="list-group-item px-0 text-muted">Sem sub tarefas.</li><?php endif; ?></ul></div><div class="col-md-6"><h3 class="h6">Checklist</h3><ul class="list-group list-group-flush"><?php foreach ($checklistByTask[$task['id']] ?? [] as $item): ?><li class="list-group-item px-0"><form method="post" class="d-flex align-items-center gap-2"><input type="hidden" name="action" value="toggle_checklist"><input type="hidden" name="item_id" value="<?= (int) $item['id'] ?>"><input type="hidden" name="is_done" value="<?= $item['is_done'] ? 0 : 1 ?>"><input class="form-check-input" type="checkbox" onchange="this.form.submit()" <?= $item['is_done'] ? 'checked' : '' ?>><span class="<?= $item['is_done'] ? 'text-decoration-line-through text-muted' : '' ?>"><?= h($item['content']) ?></span></form></li><?php endforeach; ?><?php if (empty($checklistByTask[$task['id']])): ?><li class="list-group-item px-0 text-muted">Sem checklist.</li><?php endif; ?></ul></div></div></div></div>
        <?php endforeach; ?>
